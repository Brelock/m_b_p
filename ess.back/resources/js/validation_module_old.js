...
glob.ValidationModule = (function() {
  ...
  function hideWarning({target}) {
    target.classList.remove('js_containsError');
    const next = target.nextElementSibling;
    if (next && next.classList.contains('js_warning-list')) {
      next.classList.add('js_hide');
    }
  }

  function isValid(formElement) {
    const form = formElement instanceof jQuery ? formElement[0] : formElement;
    let inputsForValidate = form.querySelectorAll('[data-validate]:not(:disabled)');
    let inputs = [];
    for (let input of inputsForValidate) {
      if (!input.hasAttribute('skip-validate')) {
        input.onfocus = hideWarning;
        inputs.push(input);
      }
    }
