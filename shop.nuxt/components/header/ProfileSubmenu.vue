<template>
	<div :class="['sub-menu auth-sub-menu parent-width for-user', {'js_hide':isLoading}]" v-if="authUser">
		<div class="sub-menu-container">
			<ul class="group">
				<li class="capitalize-first"><nuxt-link :to="localePath('/profile')">{{ $t('menu.yourProfile') }}</nuxt-link></li>
				<li class="capitalize-first"><nuxt-link :to="localePath('/profile/orders')">{{ $t('common.orders') }}</nuxt-link></li>
				<li class="capitalize-first"><nuxt-link :to="localePath('/profile/reviews')">{{ $t('common.reviews') }}</nuxt-link></li>
			</ul>

			<ul class="group">
				<li>
					<i class="icomoon icon-exit1"></i>
					<span class="hover primary capitalize-first" @click="handleLogout">{{ $t('menu.logout') }}</span>
				</li>
			</ul>
		</div>
	</div>
