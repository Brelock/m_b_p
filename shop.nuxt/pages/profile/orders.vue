								...
								<div>
									<div class="imgWrapper">
										<img :src="getProdImgSrc(item.product.pictures[0],
											item.product.id/*, 'picture_thumb_file_name'*/)" alt="prod img error" >
										<!-- <img src="./img/products/prod-1.jpg" alt="prod"> -->
									</div>
								</div>
																		
								<div>
									<div class="grey-highlight">{{ item.product.category[`title_${$i18n.locale}`] }}</div>
									<div class="">
										<nuxt-link
											:to="localePath({
												name:'catalog',
											...
											v-text="item.product[`title_${$i18n.locale}`]"
										/>
									</div>
								</div>
...
<script>
	...
	import { getProdImgSrc, cleanDateString } from '@/helpers';
