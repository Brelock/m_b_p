...
const getters = {
	flatCategoriesList: ({catalogData}) => {
		// let result = [];
		...
		return getChildrenCat(catalogData)
	},
};
...
const actions = {
	...
	fetch_catalog_items(storeArgs, payload = {}) {
		const newPayload = { ...payload,
			method: 'GET',
			notNotify: true,
			// notSetToStore: true,
			stateProp: 'catalogData',
		...
		/*multipurpose_response(storeArgs, `/categories`, newPayload).then(categories => {
			multipurpose_response(storeArgs, `/products`, newPayload).then(products => {
				...
				const mutationPayload = { stateProp: 'catalogData', value: items };
