...
const actions = {
			...
			.then(response => {
				...
				if (isSuccessStatus(response)) {
					...
					if (access_token) {
						...
						Notification.success({/*title: 'sdfsdf',*/ message: `${i18n.t('phrase.welcome')} ${user.full_name}`/*, duration: 0*/});
					} else {
	...
	sign_in_social({ commit, dispatch }, payload) {
		...
		api(this.$axios, 'GET', `/social-auth/${payload.provider}`, payload)
			.then(response => {
				// console.log(response)
				// console.log('sign_in', response.headers['cookie-hash'])
	...
	get_auth_user({ commit, dispatch }, payload = {}) {
		commit('SET_STATUS_LOADING', true);
		const { token, notifyWhenSuccess } = payload;
		if (token) {
			...
			api(this.$axios, 'GET', '/auth/user')
				...
				.catch(error => {
					...
					if (notifyWhenSuccess) {
						...
						Notification.error({ title: `Error`, message: message || 'error' });
					}
					// console.log(error)
					/*handleError(error, {
						commit: commit,
