...
const setupCategoryCrumbsList = (category, categoriesList) => {
	...
	if (parentCatsList.length) {
		...
		const homeItem = {id:1, to:'/', title_ru:'Главная', title_uk:'Головна' };
...
const getPrice = (price, quantity) => {
	...
	if ( (num ^ 0) === num ) {
		return `${num * quantity} грн`;
	} else {
		return `${(num * quantity).toFixed(2)} грн`;
	}
};
...
const getSeoLink = route => {
	return `https://api.zengi.zengineers.company${route.path				
		.toLowerCase()
		.replace(/\/$/, '')}`
};
