...
// реквизиты и финансовая отчетность
    ...
    Breadcrumbs::for('calculator', function ($trail) {
        $trail->parent('site.home');
        $trail->push(trans('main.credit_calculator'), route('calculator'));
    });

    /**спецвозможности**/

    Breadcrumbs::for('special.abilities', function ($trail) {
        $trail->parent('site.home');
        $trail->push(trans('main.special_abilities'), route('special.abilities'));
    });
...
//Промо Запись
Breadcrumbs::for('promo.show', function ($trail, $promo) use ($locale) {
    $promo = Promo::findOrFail($promo);
    $trail->parent('site.home');
    $trail->push($promo['title_' . $locale], route('promo.show', $promo));
});
