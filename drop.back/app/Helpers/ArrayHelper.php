...
class ArrayHelper {
  ...
  public static function getNotEmptyValue($array, $key, $default = null) {
    ...
    return !RequestHelper::isEmpty($value) ? $value : $default;
  }
   ...
   * @return array
   */
  public static function createArray(int $iteration) {
    $array = [];
   ...
   * @return array
   */
  public static function createArrayByRange(int $min, int $max) {
    $array = [];
    for ($i = $min; $i <= $max; $i++)
      $array[$i] = $i;
   ...
   * @return bool
   */
  public static function hasAllInArray(array $array) : bool {
    $notEmptyItems = Arr::where($array, function($value) {
      return !empty($value);
    });

    return count($notEmptyItems) === count($array);
  }
