...
@section('js_after')
    ...
    <script>
        $('#category-modal').on('show.bs.modal', function(e) {
            ...
            var createAction = "{{ route('admin.subcategories.store') }}";

            if( !titleRU && !slug){
                $(e.currentTarget).find('#titleModal').text('Создать подкатегорию :');
                ...
                $(e.currentTarget).find('#parent-category').val(0);
            } else{
                $(e.currentTarget).find('#titleModal').text('Редактировать подкатегорию :');
                ...
                $(e.currentTarget).find('#parent-category').val(parent);
            }
...
{{--                            <th style="width: 100px;">ФОТО</th>--}}
                            ...
                            <th class="">Родительская категория</th>
                            <th class="">Слаг</th>
                            <th class="">Статус</th>
...
{{--                                </td>--}}
                                ...
                                <td class="d-none d-sm-table-cell">
                                    ...
                                    <button type="button"
                                            ...
                                            data-parent="{{$category->parent->id ?? 0 }}"
                                            data-toggle="modal"
            ...
            <div class="modal-content">
                <div class="block block-themed block-transparent mb-0">
                    <div class="block-header bg-primary-info">
                        <h3 class="block-title"><span id="titleModal">Редактировать подкатегорию :</span></h3>
                                    ...
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                            <div class="form-material">
                                                <select type="text" class="form-control" id="parent-category" name="parent_id" required>
                                                    @foreach($parentCategories as $parent)
                                                        <option value="{{ $parent->id }}">{{ $parent->title }}</option>
                                                    @endforeach
                                                </select>
                                                <label for="category-title-ru">Родительская категория</label>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-row">
