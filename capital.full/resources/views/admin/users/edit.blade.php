        ...
        <div class="row">
            ...
            <div class="col-sm-3">
                <div class="form-group">
                    ...
                    <select name="published" id="" class="form-control" @if(auth()->id() == $user->id) disabled @endif>
                ...
                <div class="form-group">
                    ...
                    <select name="role" id="" class="form-control" @if(auth()->id() == $user->id) disabled @endif>
                        <option value="{{ $user::MANAGER }}" @isset($user->role) @if($user->role == $user::MANAGER ) {{ 'selected' }} @endif @endisset >
                            Техно менеджер
                        </option>
                        <option value="{{ $user::SPECIAL_MANAGER }}" @isset($user->role) @if($user->role == $user::SPECIAL_MANAGER ) {{ 'selected' }} @endif @endisset >
                            Спец менеджер
                        </option>
                        <option value="{{ $user::ADMIN }}" @isset($user->role) @if($user->role == $user::ADMIN) {{ 'selected' }} @endif @endisset>
                            Админ
                        </option>
                    </select>
                </div>
