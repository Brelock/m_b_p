        ...
        <div class="navigate d-flex justify-content-between align-items-start">
            <div class="buttons">
                <div class="button-group form-group">
                    ...
                    <a href="{{ route('offices.export.evaluations') }}">
                        <button type="button" class="btn btn-primary"><i class="fa fa-download" aria-hidden="true"></i>Excel (отзывы)</button>
                    </a>
                    <a href="{{ route('offices.export.schedules') }}">
                        <button type="button" class="btn btn-primary"><i class="fa fa-download" aria-hidden="true"></i>Excel (графики работы)</button>
                    </a>
                </div>
            </div>
        ...
        <table class="table table-striped">
            ...
            @foreach($offices as $office)
                <tr data-element-id="{{ $office->id }}" @if(session()->has('marked') and $office->id == session('marked'))  class="table-success" @endif>
                    ...
                    <td>{{ $office->street_type_ru }}</td>
                    <td>{{ $office->address_ru }}</td>
                    <td>{{ $office->phone }}</td>
                    <td>{{ $office->created_at }}</td>
                    <td>
                        @if($office->evaluations_count != 0)
                            <a href="{{ route('offices.show.comments', ['office' => $office->id]) }}">
                                <i class="fa fa-eye" aria-hidden="true"></i></a>
                            ({{ $office->evaluations_count }})
                        @else
                            нет отзывов
                        @endif
                    </td>
                    <td>
                        @forelse($office->evaluations->groupBy('mark') as $mark => $evaluations)
                            @if($mark == 1)
                                <span class="badge badge-success">{{ count($evaluations) }}</span>
                            @elseif($mark == 0)
                                <span class="badge badge-danger">{{ count($evaluations) }}</span>
                            @endif
                        @empty
                            нет оценок
                        @endforelse
