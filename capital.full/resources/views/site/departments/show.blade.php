                ...
                <div class="sectionBlock contentBlock flex wrap">
                    ...
                    <div class="sectionBlockColumn_sm department-description mcol-xs-12 mcol-sm-6">
                      ...
                      <span class="row-with-icon">
                        ...
                        <span>@if($locale == 'ru') г.@else м.@endif {{ $department->city['title_'.$locale] }}, {{ $department['street_type_'.$locale] }} {{ $department['address_'.$locale] }}</span>
                      </span>
                                </li>
                                <li>
                      <span class="row-with-icon">
                        <i class="icomoon icon-phone"></i>
                        <a href="tel:{{ $department->isTransported() ? $department->temporaryOffice->phone : $department->phone }}">{{ $department->isTransported() ? $department->temporaryOffice->phone : $department->phone }}</a>
                      </span>
                                </li>
                                <li>
                      <span class="row-with-icon">
                        ...
                        <span>
                            @if($department->isTransported())
                                {{ __('main.moved_in').'№'.$department->temporaryOffice->number.', '.$department->temporaryOffice['street_type_'.$locale].$department->temporaryOffice['address_'.$locale].', '}}
                                @if($department->temporaryOffice->full_day)
                                    {{ __('main.full_day') }}
                                @else
                                    @if($department->temporaryOffice->time_start)
                                        {{ $department->temporaryOffice->time_start }}-{{ $department->temporaryOffice->time_end }}
                                    @elseif(!$department->temporaryOffice->time_start and $department->temporaryOffice['work_days_'.$locale])
                                        {{ $department->temporaryOffice['work_days_'.$locale] }}
                                    @else
                                        Время не укзано
                                    @endif
                                @endif
                            @else
                                @if($department->full_day)
                                    {{ __('main.full_day') }}
                                @else
