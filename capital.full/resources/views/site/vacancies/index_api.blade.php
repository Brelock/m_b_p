...
@include('site.includes.meta_tags', array('meta_tags' => $static_part))
            ...
            <div class="mcontainer">
                ...
                <div class="sectionBlock">
                    ...
                    <div class="description mcol-xs-12 mcol-md-7">
                        {!! $static_part['description_'.$locale] !!}
                    </div>
                </div>
                ...
                <div class="sectionBlock filter-block flex wrap center">
                    <div class="formRow chosen-wrapper mcol-xs-12 mcol-sm-6">
                        ...
                        <select name="city" id="city_id" data-placeholder="Все" class="mcol-xs-12 mcol-sm-4">
                            ...
                            @foreach($citiesArray as $id => $city)
                                <option value="{{ $id }}"  @if(array_get($request, 'city') == $id) {{ 'selected' }} @endif>{{ $city }}</option>
{{--                                <option value="{{ $id }}" @if(array_get($request, 'city') == $city->id) {{ 'selected' }} @endif>{{ $city['title_'.$locale] }}</option>--}}
                            @endforeach
                        </select>
                    </div>
                        ...
                        <li>
                            <div class="header-block showButton"
                                ...
                                <div class="content-container">
                                    <h3 class="title">{{ $vacancy->name }}</h3>
                                    <div class="sub-title vacancy-info">
                                        <div><span>{{ $citiesArray[$vacancy->cityId] }}</span></div>
                                        <div><span>{{ \Jenssegers\Date\Date::parse($vacancy->date)->format('d m Y') }}</span></div>
                                            ...
                                            <div class="more-button credit-button inversed">
                                                <div class="more-button-wrapper">
                                                    <div class="more-button-container">
                                                        <a href="https://rabota.ua/company775048/vacancy{{ $vacancy->id }}" rel="nofollow" target="_blankпше " class="title semi-bold">{{ __('main.send_resume') }}</a>
        ...
        $(document).ready(function () {
            ...
            city.chosen().change(function () {
                if ($(this).val() === 'national') {
                    ...
                    url = category.chosen().val() !== 'all' ? '/vacancies?city=' + city.chosen().val() : '/vacancies?city=' + city.chosen().val() ;
                }
            ...
            //         url = $(this).val() === 'all' ? '/vacancies?national=1&all=1' : '/vacancies?national=1';
            //     }else {
            //         url = city.chosen().val() === 'all' ? '/vacancies?city='+ city.chosen().val() +'&all=1' : '/vacancies?city='+ city.chosen().val();
            //     }
            //     window.location.assign(url);
            // });
        });
