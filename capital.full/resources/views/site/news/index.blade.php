...
@section('content')
    ...
    {{--                                                        href="{{ route('news.show',  $item->alias) }}">{{ $item['title_'.$locale] }}</a>--}}
                                                    href="{{ route('news.show',  $item->alias) }}">{{ html_entity_decode($item['title_'.$locale]) }}</a>
                                        </h4>
                                        <div class="description">
                                            <p class="">
                                                {{--{!! strip_tags(str_limit($item['description_'.$locale], 155)) !!}--}}
                                                {!! substr(strip_tags($item['description_'.$locale]), 0, strrpos(substr(strip_tags($item['description_'.$locale]), 0, 200), ' ')); !!} ...
                                            </p>
                                        </div>
                                    </div>
                                </div>
